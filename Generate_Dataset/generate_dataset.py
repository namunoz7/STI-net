...
def gen_cylinders_v2(n_cylinders):
    ...
    vec_radius, vec_alpha, vec_beta, vec_omega = rp.rand_parameters_cylinders(n_cylinders)
...
def gen_spheres_v2(n_spheres):
    ...
    :return:
    """
    vec_radius, vec_alpha, vec_beta, vec_omega = rp.rand_parameters_cylinders(n_spheres)
...
def get_total_phase(chi, vec_theta, vec_psi):
    ...
    real_chi, img_chi = ft.fft_phase(chi)
    tmp_real = torch.matmul(matrix_projection, real_chi)
    tmp_img = torch.matmul(matrix_projection, img_chi)

    phase = ft.inv_fft_phase(torch.cat((tmp_real, tmp_img), dim=-1))
    return phase[:, :, :, :, 0], matrix_projection
