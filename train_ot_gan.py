...
CHECKPOINT_FOLDER = 'checkpoints/Pytorch/STI_real_data_1_11.3/'
...
class GeometricFiguresDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        with h5py.File(filename, 'r') as hf:
            out_tensor = torch.tensor(numpy.array(hf.get(self.key)))
            out_tensor = out_tensor.to(torch.float32).to(DEVICE)
            return out_tensor


# Dataset
chi_dataset = GeometricFiguresDataset(dataset_dir=CHI_FOLDER, key_dataset='chi', num_dataset=NUM_CHI)
mat_dataset = GeometricFiguresDataset(dataset_dir=MAT_FOLDER, key_dataset='A', num_dataset=NUM_MAT)
...
def training_step(chi, matrix_projection):
    """
...
def val_step(chi, matrix_projection):
    """
...
def main():
    ...
    for n_epoch in range(NUM_EPOCHS):
        ...
        for n_mat, mat_projection in enumerate(mat_loader, actual_mat):
            # torch.cuda.empty_cache()
            print('Mat number ' + str(n_mat))
            n_img = np.random.randint(0, BATCH_SIZE)
            # TRAINING PHASE
            print('Training phase')
            sti_net.train()
            mat_projection = mat_projection.to(DEVICE)
            mat_projection = mat_projection.repeat(BATCH_SIZE, 1, 1, 1, 1, 1)
            actual_loss_gen = 0.0
            iso_train_loss = 0.0
            ani_train_loss = 0.0
            tmp = 0
            ...
            train_writer.add_scalars(main_tag='Train_Loss',
                                     ...
                                     global_step=n_iter)
            train_writer.add_scalar(tag='Generator_Train_Loss', scalar_value=train_loss[n_iter],
                                    global_step=n_iter)
            train_writer.close()
            train_writer.flush()
            ...
            for n_chi, chi in enumerate(val_loader):
                ...
                tmp += 1
                chi = chi.to(DEVICE)
                [iso_gen_loss, ani_gen_loss, gen_val_loss] = val_step(chi, mat_projection)
