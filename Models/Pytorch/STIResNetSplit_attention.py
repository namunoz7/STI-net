...
class STIResNetSplit(nn.Module):
    class BottleneckAttention(nn.Module):
        def __init__(self, in_channels, reduction_features=16):
            ...
            self.spatial_attention = nn.Conv3d(in_channels, 1, kernel_size=1, stride=1, dtype=DTYPE)
        ...
        def forward(self, x, parameter_angles):
            ...
            return channel_weights * spatial_weights * x
    ...
    # def __init__(self, in_channels=6, out_channels=(3, 3), init_features=32, stride=2):
    def __init__(self, in_channels=6, out_channels=6, init_features=32, stride=2):
        ...
        # Decoders
        # ------------------------------------
        self.decoder_chi = self.Decoder(features=features, out_channels=out_channels)
        self.decoder_pev = self.Decoder(features=features, out_channels=3)
    ...
    def forward(self, x, parameter_angles):
        ...
        # Decoders
        chi = self.decoder_chi(bottleneck, enc0, enc1, enc2, enc3, enc4)
        pev = self.decoder_pev(bottleneck, enc0, enc1, enc2, enc3, enc4)
    ...
    def _enc_block(in_channels, features, name, stride):
        return nn.Sequential(
            OrderedDict(
                [
                    (name + "_norm1", nn.BatchNorm3d(num_features=in_channels, device=DEVICE, dtype=DTYPE)),
                    (name + "_conv1", nn.Conv3d(in_channels=in_channels, out_channels=features, stride=stride,
                                                kernel_size=(3, 3, 3), padding=1, bias=False,
                                                device=DEVICE, dtype=DTYPE)),
                    (name + "_relu1", nn.LeakyReLU(negative_slope=0.2, inplace=True)),

                    (name + "_norm2", nn.BatchNorm3d(num_features=features, device=DEVICE, dtype=DTYPE)),
    ...
    def _encoder_0(input_layer, features=32):
        return nn.Sequential(
            nn.Conv3d(in_channels=input_layer, kernel_size=(3, 3, 3), out_channels=features,
                      padding=1, bias=False, device=DEVICE, dtype=DTYPE),
            nn.BatchNorm3d(num_features=features, device=DEVICE, dtype=DTYPE),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
    ...
    def _up_sample_block(in_layers, out_layers):
        return nn.Sequential(
            ...
            nn.Conv3d(in_channels=in_layers, out_channels=out_layers,
