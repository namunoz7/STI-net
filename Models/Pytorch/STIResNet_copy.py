...
class STIResNetSplit(nn.Module):
    class BottleneckAttention(nn.Module):
        ...
        def forward(self, x, parameter_angles):
            ...
            # tmp = self.avg_pool(x).squeeze()
            print("Size of self.avg_pool(x).squeeze(): %3d" % self.avg_pool(x).squeeze().size())
            print(parameter_angles.size())
            x1 = torch.cat((self.avg_pool(x).squeeze(), parameter_angles), dim=0)
