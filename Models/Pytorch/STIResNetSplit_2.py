...
class STIResNetSplit(nn.Module):
    def __init__(self, in_channels=6, out_channels=(3, 3), init_features=32, stride=(2, 2, 2)):
        ...
        # Bottleneck
        # ------------------------------------
        self.bottleneck = STIResNetSplit._enc_block(16 * features, 32 * features, name="bottleneck", stride=(2, 2, 2))

        # ------------------------------------
        # Isotropic decoder
        # ------------------------------------
        self.up_conv4_i = STIResNetSplit._up_sample_block(in_layers=32*features, out_layers=16*features)
        self.decoder4_i = STIResNetSplit._dec_block(2 * (16 * features), 16 * features, name="dec4_i")
        self.up_conv3_i = STIResNetSplit._up_sample_block(in_layers=16*features, out_layers=8*features)
        self.decoder3_i = STIResNetSplit._dec_block(2 * (8 * features), 8 * features, name="dec3_i")
        self.up_conv2_i = STIResNetSplit._up_sample_block(in_layers=8*features, out_layers=4*features)
        self.decoder2_i = STIResNetSplit._dec_block(2 * (4 * features), 4 * features, name="dec2_i")
        self.up_conv1_i = STIResNetSplit._up_sample_block(in_layers=4*features, out_layers=2*features)
        self.decoder1_i = STIResNetSplit._dec_block(2 * (2 * features), 2 * features, name="dec1_i")
        self.up_conv0_i = STIResNetSplit._up_sample_block(in_layers=2*features, out_layers=features)

        self.conv_i = nn.Sequential(nn.Conv3d(in_channels=features, out_channels=out_channels[0],
        ...
        # Anisotropic decoder
        # ------------------------------------
        self.up_conv4_a = STIResNetSplit._up_sample_block(in_layers=32*features, out_layers=16*features)
        self.decoder4_a = STIResNetSplit._dec_block(2 * (16 * features), 16 * features, name="dec4_a")
        self.up_conv3_a = STIResNetSplit._up_sample_block(in_layers=16*features, out_layers=8*features)
        self.decoder3_a = STIResNetSplit._dec_block(2 * (8 * features), 8 * features, name="dec3_a")
        self.up_conv2_a = STIResNetSplit._up_sample_block(in_layers=8*features, out_layers=4*features)
        self.decoder2_a = STIResNetSplit._dec_block(2 * (4 * features), 4 * features, name="dec2_a")
        self.up_conv1_a = STIResNetSplit._up_sample_block(in_layers=4*features, out_layers=2*features)
        self.decoder1_a = STIResNetSplit._dec_block(2 * (2 * features), 2 * features, name="dec1_a")
        self.up_conv0_a = STIResNetSplit._up_sample_block(in_layers=2*features, out_layers=features)

        self.conv_a = nn.Sequential(nn.Conv3d(in_channels=features, out_channels=out_channels[0],
                                              kernel_size=(3, 3, 3), padding=1, device=DEVICE),
                                    nn.LeakyReLU(negative_slope=0.2, inplace=True))
    ...
    def forward(self, x):
        ...
        print('Encoder 4 size: ' + str(enc4.shape))

        # Bottleneck
        bottleneck = self.bottleneck(enc4)
        print('Bottleneck size: ' + str(bottleneck.shape))

        # Isotropic Decoder
        dec4_i = self.up_conv4_i(bottleneck)
        print('Up-sample 4 decoder size: ' + str(dec4_i.shape))
        dec4_i = torch.cat((dec4_i, enc4), dim=1)
        print('Up-sample 4 after skip connection: ' + str(dec4_i.shape))
