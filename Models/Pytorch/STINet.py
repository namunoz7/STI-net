...
class STINet(nn.Module):
    def __init__(self, in_channels=6, out_channels=6, init_features=32):
        super(STINet, self).__init__()
        features = init_features
        self.encoder1 = STINet._block(in_channels, features, name="enc1")
        self.pool1 = nn.MaxPool3d(kernel_size=2, stride=2)
        self.encoder2 = STINet._block(features, 2 * features, name="enc2")
        self.pool2 = nn.MaxPool3d(kernel_size=2, stride=2)
        self.encoder3 = STINet._block(2 * features, 4 * features, name="enc3")
        self.pool3 = nn.MaxPool3d(kernel_size=2, stride=2)
        self.encoder4 = STINet._block(4 * features, 8 * features, name="enc4")
        self.pool4 = nn.MaxPool3d(kernel_size=2, stride=2)

        self.bottleneck = STINet._block(8 * features, 16 * features, name="bottleneck")

        self.upconv4 = nn.ConvTranspose3d(16 * features, 8 * features, kernel_size=2, stride=2)
        self.decoder4 = STINet._block(2 * (8 * features), 8 * features, name="dec4")
        self.upconv3 = nn.ConvTranspose3d(8 * features, 4 * features, kernel_size=2, stride=2)
        self.decoder3 = STINet._block(2 * (4 * features), 4 * features, name="dec3")
        self.upconv2 = nn.ConvTranspose3d(4 * features, 2 * features, kernel_size=2, stride=2)
        self.decoder2 = STINet._block(2 * (2 * features), 2 * features, name="dec2")
        self.upconv1 = nn.ConvTranspose3d(2 * features, features, kernel_size=2, stride=2)
        self.decoder1 = STINet._block(2 * features, features, name="dec1")
