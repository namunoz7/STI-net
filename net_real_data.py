...
from Models.Pytorch.STIResNet_copy import STIResNetSplit
...
if IS_COSMOS:
    ...
    RESULTS_FOLDER = os.path.join(SIMULATED_FOLDER, 'Real_data/STI_partial_3')
...
def load_dir_field(dir_field_file, directions):
    ...
    dir_field = torch.tensor(dir_field[directions, :], dtype=torch.float32).reshape(18)
    return dir_field
...
def main():
    ...
    t = time()
    chi_model = sti_model(phi.unsqueeze(0), direction_field)
    t2 = time() - t
